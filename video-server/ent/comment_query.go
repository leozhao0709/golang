...
func (cq *CommentQuery) QueryVideo() *VideoQuery {
	query := &VideoQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) WithVideo(opts ...func(*VideoQuery)) *CommentQuery {
	query := &VideoQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context) ([]*Comment, error) {
	...
	if cq.withAuthor != nil || cq.withVideo != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withVideo; query != nil {
