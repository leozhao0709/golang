...
func (vq *VideoQuery) QueryComments() *CommentQuery {
	query := &CommentQuery{config: vq.config}
...
func (vq *VideoQuery) QueryAuthor() *UserQuery {
...
func (vq *VideoQuery) WithAuthor(opts ...func(*UserQuery)) *VideoQuery {
	query := &UserQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VideoQuery) sqlAll(ctx context.Context) ([]*Video, error) {
	...
	if query := vq.withAuthor; query != nil {
...
func (vq *VideoQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := vq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, video.ValidColumn)
			}
		}
	}
	return _spec
...
func (vgb *VideoGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range vgb.fns {
		columns = append(columns, fn(selector, video.ValidColumn))
	}
